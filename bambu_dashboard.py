...
CHANGELOG = [
    {"versao": "5.17", "data": "2026-08",
     "titulo": "Correcao importante na licenca",
     "novidades": [
         "Corrigido o problema em que o sistema pedia a licenca de novo sem motivo.",
         "O codigo da maquina agora e reconhecido mesmo se a leitura oscilar.",
         "Uma vez ativado, o sistema nao volta mais para a tela de ativacao a toa.",
     ]},
    {"versao": "5.16", "data": "2026-08",
     "titulo": "Ajuste no cadastro de filamento",
     "novidades": [
         "Seletor kg / gramas nao sobrepoe mais o campo de preco.",
         "Campos do cadastro de filamento com tamanho mais equilibrado.",
     ]},
    {"versao": "5.15", "data": "2026-08",
     "titulo": "Correcao no peso do filamento (virgula) e telas",
     "novidades": [
         "Agora voce pode digitar o peso e o preco com virgula (ex: 1,5 kg) ou com ponto.",
         "Novo seletor kg / gramas no cadastro de filamento (mais a prova de erro).",
         "Antes, ao usar virgula, o filamento era cadastrado com peso zero. Corrigido.",
         "Campos de preenchimento nao ficam mais cortados em telas menores.",
     ]},
    {"versao": "5.14", "data": "2026-08",
     "titulo": "Ativacao pela Central de Licencas",
     "novidades": [
         "A chave de ativacao agora e validada pela Central de Licencas.",
         "Cada chave trava em uma maquina (seguranca).",
     ]},
    {"versao": "5.13", "data": "2026-08",
     "titulo": "Diagnostico Flashforge (serie)",
     "novidades": [
         "Ferramenta interna para descobrir o numero de serie correto da Flashforge.",
     ]},
    {"versao": "5.12", "data": "2026-08",
     "titulo": "Custo da impressao mais flexivel",
     "novidades": [
         "Ao desmarcar \"Descontar do estoque\", aparece o campo para informar o custo do filamento manualmente.",
     ]},
    {"versao": "5.11", "data": "2026-08",
     "titulo": "Flashforge: numero de serie",
     "novidades": [
         "A tela agora explica onde achar o numero de serie (SN) da Flashforge.",
         "O SN e obrigatorio e deve ser digitado com o SN na frente.",
     ]},
    {"versao": "5.10", "data": "2026-08",
     "titulo": "Diagnostico Flashforge",
     "novidades": [
         "Ferramenta interna para ajustar a leitura das impressoras Flashforge.",
     ]},
    {"versao": "5.9", "data": "2026-08",
     "titulo": "Central de Licencas (verificacao online)",
     "novidades": [
         "O sistema agora confirma a assinatura ativa com a Central.",
         "Se a assinatura for cancelada, o acesso e suspenso.",
         "Queda de internet nao bloqueia o sistema.",
     ]},
    {"versao": "5.8", "data": "2026-08",
     "titulo": "Ajustes finos",
     "novidades": [
         "Mensagem clara ao tentar ver camera de impressora que nao tem camera.",
         "Pequena limpeza interna no cadastro de impressoras.",
     ]},
...
def machine_fingerprint_set():
    """Conjunto de códigos que representam ESTA máquina (o principal + cada
    componente de hardware sozinho). Enviar todos para a Central permite que
    ela reconheça a máquina mesmo se a leitura de um componente oscilar —
    é a MESMA tolerância que a licença offline já usa. Retorna lista de str.
    Nunca lança exceção."""
    fps = set()
    try:
        principal = machine_fingerprint()
        if principal:
            fps.add(principal)
    except Exception:
        pass
    # tenta o conjunto tolerante do núcleo (uuid sozinho, board sozinho, etc.)
    try:
        if _CORE_OK and hasattr(_core, "_fingerprint_set"):
            for f in _core._fingerprint_set():
                if f:
                    fps.add(f)
    except Exception:
        pass
...
# ═══════════════════════════════════════════════════════════════════
#  CENTRAL DE LICENÇAS (verificação online)
#  Além da licença offline (núcleo), o sistema consulta a Central para
#  saber se o cliente continua ativo. Regras:
#   - Central diz "inativo"  → BLOQUEIA
#   - Central diz "ativo"    → libera (e guarda o horário como "último ok")
#   - Central inacessível    → NÃO bloqueia (internet caiu não é motivo).
#     Só bloqueia por falta de resposta se ficar MUITOS dias sem NENHUM
#     contato bem-sucedido (proteção contra cliente que corta a internet
#     de propósito para sempre). Esse limite é largo e tolerante.
# ═══════════════════════════════════════════════════════════════════
import hmac as _hmac_mod
import hashlib as _hashlib_mod

CENTRAL_CONFIG_PATH = Path(__file__).with_name("central_config.json")
# Estado da verificação online (memória + persistido)
CENTRAL_STATE = {"liberado": True, "motivo": "sem_verificacao", "ultimo_ok": None,
                 "verificado_em": None}
# Dias que o sistema tolera SEM nenhuma resposta positiva da Central antes de,
# por precaução, pedir reconexão. Largo de propósito (não pune queda de internet).
CENTRAL_MAX_DIAS_OFFLINE = 14


def _central_config():
    """Lê a config da Central: {url, licenca_id, secret}."""
    try:
        if CENTRAL_CONFIG_PATH.exists():
            return json.loads(CENTRAL_CONFIG_PATH.read_text(encoding="utf-8"))
    except Exception:
        pass
    return {}


def _ja_ativado_alguma_vez():
    """True se este sistema já foi ativado pela Central em algum momento
    (existe um licenca_id salvo no central_config). Uma vez ativado, o sistema
    NÃO deve voltar para a tela de ativação por conta de oscilação do código da
    máquina — quem controla bloqueio a partir daí é a Central (tela 'suspenso').
    Isso evita pedir a licença de novo sem motivo."""
    try:
        cfg = _central_config()
        return bool(cfg.get("url") and cfg.get("licenca_id"))
    except Exception:
        return False


def _central_salvar_estado():
    try:
        CENTRAL_CONFIG_PATH  # noqa
        estado_path = Path(__file__).with_name("central_estado.json")
        estado_path.write_text(json.dumps(CENTRAL_STATE), encoding="utf-8")
    except Exception:
        pass


def _central_carregar_estado():
    global CENTRAL_STATE
    try:
        estado_path = Path(__file__).with_name("central_estado.json")
        if estado_path.exists():
            CENTRAL_STATE.update(json.loads(estado_path.read_text(encoding="utf-8")))
    except Exception:
        pass


def verificar_central():
    """Consulta a Central de Licenças. Atualiza CENTRAL_STATE.
    Retorna True se pode funcionar, False se a Central mandou bloquear."""
    cfg = _central_config()
    url = cfg.get("url", "").rstrip("/")
    licenca_id = cfg.get("licenca_id", "")
    secret = cfg.get("secret", "")

    # Se a Central não está configurada, não bloqueia (modo offline puro).
    if not url or not licenca_id:
        CENTRAL_STATE["liberado"] = True
        CENTRAL_STATE["motivo"] = "central_nao_configurada"
        return True
    ...
    try:
        ...
        r = _rq.post(f"{url}/api/verificar",
                     json={"licenca_id": licenca_id, "fingerprint": fp,
                           "fingerprints": machine_fingerprint_set()},
                     timeout=8)
        if r.status_code == 200:
            d = r.json()
            # valida o ticket (assinatura HMAC) para garantir que a resposta
            # veio mesmo da Central, e não de um impostor/DNS sequestrado.
            if secret:
                recebido = d.get("ticket", "")
                corpo = {k: d[k] for k in ("liberado", "motivo", "ts") if k in d}
                calc = _hmac_mod.new(secret.encode(),
                                     json.dumps(corpo, sort_keys=True,
                                                separators=(",", ":")).encode(),
                                     _hashlib_mod.sha256).hexdigest()
                if not _hmac_mod.compare_digest(calc, recebido):
                    # resposta não confiável → por segurança, não bloqueia,
                    # mas também não registra como "ok" (trata como offline)
                    CENTRAL_STATE["motivo"] = "resposta_nao_confiavel"
                    return _central_avaliar_offline()

            liberado = bool(d.get("liberado"))
            CENTRAL_STATE["liberado"] = liberado
            CENTRAL_STATE["motivo"] = d.get("motivo", "")
            CENTRAL_STATE["verificado_em"] = datetime.now(timezone.utc).isoformat()
            if liberado:
                CENTRAL_STATE["ultimo_ok"] = datetime.now(timezone.utc).isoformat()
            _central_salvar_estado()
            return liberado
        else:
            # erro no servidor → trata como offline (não pune o cliente)
            return _central_avaliar_offline()
    except Exception:
        # sem internet / central fora do ar → NÃO bloqueia
        return _central_avaliar_offline()


def _central_avaliar_offline():
    """Quando não conseguiu falar com a Central: decide sem puni-lo por queda
    de internet, mas com um limite largo contra abuso permanente."""
    ultimo = CENTRAL_STATE.get("ultimo_ok")
    if not ultimo:
        # nunca conseguiu falar com a Central ainda → dá o benefício da dúvida
        CENTRAL_STATE["liberado"] = True
        CENTRAL_STATE["motivo"] = "central_inacessivel_sem_historico"
        return True
    try:
        d = datetime.fromisoformat(ultimo)
        dias = (datetime.now(timezone.utc) - d).days
    except Exception:
        dias = 0
    if dias > CENTRAL_MAX_DIAS_OFFLINE:
        # muito tempo sem NENHUM contato bem-sucedido → pede reconexão
        CENTRAL_STATE["liberado"] = False
        CENTRAL_STATE["motivo"] = f"sem_contato_ha_{dias}_dias"
        return False
    # dentro do período tolerado → continua funcionando
    CENTRAL_STATE["liberado"] = True
    CENTRAL_STATE["motivo"] = f"offline_tolerado_{dias}_dias"
    return True
...
def start_flashforge_printer(cfg: dict):
    ...
    def run():
        while not stop_flag.is_set():
            ...
            # Guarda os dados brutos SEMPRE (mesmo se a tradução falhar),
            # para o diagnóstico em /api/flashforge/raw funcionar.
            with STATE_LOCK:
...
async def _startup():
    ...
    # Central de Licenças: carrega o último estado e verifica em segundo plano.
    _central_carregar_estado()

    def _loop_central():
        # verifica logo ao subir e depois a cada 3 horas
        while True:
            try:
                verificar_central()
            except Exception as exc:
                print("[central] erro ao verificar:", exc)
            time.sleep(3 * 3600)  # 3 horas

    threading.Thread(target=_loop_central, daemon=True, name="central").start()
...
async def index(request: Request):
    ...
    # MAS se o sistema já foi ativado alguma vez (tem licenca_id salvo), NÃO
    # volta para a ativação por oscilação do código da máquina — a partir daí
    # quem controla o acesso é a Central (bloqueio = tela "suspenso").
    if not LICENSE_STATE.get("ok") and not _ja_ativado_alguma_vez():
        return RedirectResponse("/ativar", status_code=302)
    # Gate da Central: se a Central mandou bloquear (cancelado/inadimplente),
    # mostra a tela de acesso suspenso. Queda de internet NÃO cai aqui.
    if not CENTRAL_STATE.get("liberado", True):
        return HTMLResponse(_tela_suspenso(CENTRAL_STATE.get("motivo", "")))
    if not is_authed(request):
...
def _tela_suspenso(motivo):
    """Tela mostrada quando a Central bloqueou o acesso (não por internet)."""
    if motivo == "cancelado":
        titulo = "Assinatura cancelada"
        texto = ("Sua assinatura do FarmSync foi cancelada. "
                 "Para voltar a usar o sistema, entre em contato para reativar.")
    elif motivo.startswith("inadimplente") or motivo.startswith("sem_contato"):
        titulo = "Acesso suspenso"
        texto = ("Identificamos uma pendência na sua assinatura. "
                 "Regularize para reativar o acesso ao sistema.")
    else:
        titulo = "Acesso suspenso"
        texto = ("Seu acesso ao FarmSync está temporariamente suspenso. "
...
<style>
  body{{background:#0a0e16;color:#e8edf5;font-family:'Segoe UI',system-ui,sans-serif;
    display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;text-align:center}}
  .box{{max-width:440px;padding:2.5rem;background:#0c1119;border:1px solid #1c2535;border-radius:18px}}
  img{{height:40px;margin-bottom:1.5rem}}
  .ic{{font-size:3rem;margin-bottom:1rem}}
  h1{{font-size:1.4rem;margin:0 0 .8rem}}
  p{{color:#aeb9cc;line-height:1.6;margin-bottom:1.5rem}}
  a{{display:inline-flex;align-items:center;gap:.5rem;background:#25d366;color:#04240f;
    font-weight:700;padding:.8rem 1.5rem;border-radius:11px;text-decoration:none}}
</style></head><body>
<div class="box">
  <img src="{logo}" alt="FarmSync">
  <div class="ic">🔒</div>
  <h1>{titulo}</h1>
  <p>{texto}</p>
  <a href="https://wa.me/5512988447240" target="_blank">Falar no WhatsApp</a>
</div></body></html>"""
...
async def api_ativar(request: Request):
    body = await request.json()
    key = (body.get("chave", "") or "").strip()

    # Ativação pela Central de Licenças (nova forma).
    # A chave é o licenca_id da Central. Ativa e trava na máquina.
    cfg = _central_config()
    url = cfg.get("url", "").rstrip("/")
    secret = cfg.get("secret", "")
    if url:
        ...
        try:
            import requests as _rq
            r = _rq.post(f"{url}/api/ativar",
                         json={"licenca_id": key, "fingerprint": fp,
                               "fingerprints": machine_fingerprint_set()}, timeout=10)
            d = r.json()
            if d.get("ok"):
                # salva a licença no central_config para as próximas verificações
                novo_cfg = dict(cfg)
                novo_cfg["licenca_id"] = key.upper()
                CENTRAL_CONFIG_PATH.write_text(json.dumps(novo_cfg), encoding="utf-8")
                # marca o estado como liberado e verifica já
                CENTRAL_STATE["liberado"] = True
                CENTRAL_STATE["ultimo_ok"] = datetime.now(timezone.utc).isoformat()
                _central_salvar_estado()
                # também ativa a licença offline (compatibilidade), se houver núcleo
                try:
                    install_license(key)
                    refresh_license()
                except Exception:
                    pass
                # garante que o LICENSE_STATE fique ok mesmo sem núcleo offline
                if not LICENSE_STATE.get("ok"):
                    LICENSE_STATE["ok"] = True
                    LICENSE_STATE["cliente"] = key.upper()
                    LICENSE_STATE["fingerprint"] = fp
                return {"ok": True, "msg": "Sistema ativado com sucesso!"}
            else:
                return {"ok": False, "msg": d.get("mensagem",
                        "Não foi possível ativar. Confira a chave.")}
        except Exception:
            # Central inacessível → tenta a licença offline como plano B
            pass

    # Forma antiga (licença offline pelo núcleo), como reserva
...
def _f(v, padrao=0.0):
    # Aceita número já convertido, ou texto com vírgula (padrão brasileiro)
    # ou ponto. Ex.: "1,5" e "1.5" viram ambos 1.5.
    ...
    # Casos: "1.234,56" (BR) -> "1234.56" ; "1,5" -> "1.5" ; "1.5" -> "1.5"
    if "," in s and "." in s:
...
async def camera_stream(name: str, request: Request):
    ...
    # Flashforge (AD5X): não tem câmera embutida.
    if brand == "flashforge":
        return JSONResponse(
            {"error": "Esta impressora Flashforge não possui câmera embutida."},
            status_code=400)
...
@app.get("/api/flashforge/raw")
async def api_flashforge_raw(request: Request):
    """Diagnóstico: mostra os dados brutos que a Flashforge está enviando,
    E o número de série / código que estão sendo usados (para achar o erro)."""
    if (block := _need_auth(request)):
        return block
    resultado = {}
    with CFG_LOCK:
        cfgs = [dict(c) for c in PRINTERS_CFG if c.get("brand") == "flashforge"]
    with STATE_LOCK:
        for name, st in STATE.items():
            meta = st.get("_meta", {})
            if meta.get("brand") == "flashforge":
                cfg = next((c for c in cfgs if c["name"] == name), {})
                serial = cfg.get("serial", "")
                code = cfg.get("check_code", "")
                resultado[name] = {
                    "_config_enviada": {
                        "ip": cfg.get("ip", ""),
                        "serial_completo": serial,
                        "serial_tamanho": len(serial),
                        "serial_comeca_com_SN": serial.upper().startswith("SN"),
                        "check_code_tamanho": len(code),
                    },
                    "resposta_da_impressora": meta.get("_raw", "sem dados ainda"),
                }
    if not resultado:
        return {"aviso": "Nenhuma impressora Flashforge cadastrada. "
                         "Confira se ela esta adicionada."}
    return resultado


@app.post("/api/flashforge/testar_seriais")
async def api_flashforge_testar_seriais(request: Request):
    """Testa vários formatos do número de série para descobrir qual a
    impressora aceita. Recebe {ip, check_code, serial} e testa variações."""
    if (block := _need_auth(request)):
        return block
    ...
    if not ip or not code:
        return {"ok": False, "error": "Informe o IP e o código."}

    # gera variações do serial para testar
    base = serial.upper()
    sem_sn = base[2:] if base.startswith("SN") else base
    variacoes = []
    vistos = set()
    for cand in [serial, base, sem_sn, "SN" + sem_sn,
                 serial.lower(), sem_sn.lower()]:
        if cand and cand not in vistos:
            vistos.add(cand)
            variacoes.append(cand)

    resultados = []
    for cand in variacoes:
        detail = await asyncio.to_thread(_flashforge_detail, ip, cand, code)
        d = detail.get("detail", detail) if isinstance(detail, dict) else {}
        recusou = isinstance(d, dict) and "sn" in str(d.get("message", "")).lower()
        funcionou = isinstance(d, dict) and not recusou and (
            d.get("status") or d.get("machineStatus") or d.get("printProgress") is not None
            or "detail" in (detail or {}))
        resultados.append({
            "serial_testado": cand,
            "funcionou": bool(funcionou),
            "resposta": d if isinstance(d, dict) else str(detail),
        })
        if funcionou:
            break
...
@app.post("/api/flashforge/corrigir_serial")
async def api_flashforge_corrigir_serial(request: Request):
    """Corrige o número de série de uma Flashforge já cadastrada, sem precisar
    remover e adicionar de novo. Testa o SN antes de salvar."""
    if (block := _need_auth(request)):
        return block
    b = await request.json()
    nome = (b.get("nome") or "").strip()
    novo_serial = (b.get("serial") or "").strip()
    if not nome or not novo_serial:
        return {"ok": False, "error": "Informe o nome da impressora e o número de série."}

    # acha a config da impressora
    with CFG_LOCK:
        cfg = next((c for c in PRINTERS_CFG if c["name"] == nome
                    and c.get("brand") == "flashforge"), None)
    if not cfg:
        return {"ok": False, "error": "Impressora Flashforge não encontrada."}

    ip = cfg.get("ip", "")
    code = cfg.get("check_code", "")

    # testa o novo serial ANTES de salvar
    detail = await asyncio.to_thread(_flashforge_detail, ip, novo_serial, code)
    if detail is None:
        return {"ok": False, "error": "Não consegui falar com a impressora. Confira o IP e o Modo LAN."}
    d = detail.get("detail", detail)
    # se a impressora reclamar do SN, avisa
    if isinstance(d, dict) and d.get("message") and "sn" in str(d.get("message", "")).lower():
        return {"ok": False, "error": f"A impressora recusou este número de série: \"{d.get('message')}\". "
                "Confira se digitou completo, com o SN na frente."}
    if isinstance(d, dict) and d.get("code") not in (0, None) and d.get("message"):
        return {"ok": False, "error": f"A impressora respondeu: {d.get('message')}"}

    # deu certo → para a impressora, atualiza o serial, salva e reinicia
    handle = PRINTERS.get(nome)
    if handle and handle.get("stop"):
        handle["stop"].set()
    with CFG_LOCK:
        cfg["serial"] = novo_serial
        save_printers(PRINTERS_CFG)
    await asyncio.to_thread(start_printer, cfg)
    return {"ok": True, "info": "Número de série corrigido! As métricas devem aparecer em instantes."}
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .est-form{display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr));
    gap:.9rem; max-width:760px}
  .est-form label{display:flex; flex-direction:column; gap:.4rem; font-size:.86rem; color:var(--muted)}
  .est-form input,.est-form select{background:#0a0e16; border:1px solid var(--hair); border-radius:9px;
    padding:.55rem .7rem; color:var(--ink); font-family:inherit; font-size:.92rem;
    width:100%; min-width:0; box-sizing:border-box}
  .est-form input[type=color]{padding:.2rem; height:40px; width:64px; cursor:pointer; flex:0 0 auto}
  .est-form input:focus{outline:none; border-color:var(--live); box-shadow:0 0 0 3px rgba(79,140,255,.12)}
  /* Campo Quantidade com seletor kg/gramas ao lado (sem sobrepor) */
  .est-qtd-wrap{display:flex; gap:.4rem; align-items:stretch}
  .est-qtd-input{flex:1 1 auto; min-width:0}
  .est-unidade{flex:0 0 auto; width:auto !important; min-width:74px;
  ...
  .cf-row{display:flex; align-items:center; gap:.6rem; margin-bottom:.6rem; flex-wrap:wrap}
  .cf-row label{flex:1 1 130px; min-width:110px; font-size:.85rem; color:var(--ink)}
  .cf-row input,.cf-row select{flex:0 1 130px; width:130px; min-width:90px; max-width:100%;
    background:#0a0e16;
  ...
  /* ═══════════════════════════════════════════════════════════
     ROBUSTEZ RESPONSIVA (rede de segurança p/ qualquer resolução)
     Evita que campos sejam "comidos"/cortados em telas intermediárias.
     ═══════════════════════════════════════════════════════════ */
  *,*::before,*::after{box-sizing:border-box}
  /* Nenhum input/select/textarea estoura o container onde está */
  input,select,textarea,button{max-width:100%}
  /* Itens de grid/flex podem encolher (sem isso, o conteúdo empurra e corta) */
  .est-form label,.q-cli label,.q-cond label,.settings-grid > *,
  .cf-row label,.calc-grid > *,.hl-grid > *{min-width:0}
  .est-form input:not(.est-unidade):not([type=color]),.est-form select:not(.est-unidade),
  .q-cli input,.q-cond input,.q-obs textarea,.q-table input,.q-table select{
    width:100%; min-width:0}
  /* Telas intermediárias (notebooks ~1000-1300px): formulários com menos
     colunas para os campos respirarem */
  @media (max-width:1300px){
    .est-form{grid-template-columns:repeat(auto-fit,minmax(190px,1fr))}
  }
  @media (max-width:900px){
    .est-form{grid-template-columns:1fr 1fr}
    .q-cli{grid-template-columns:1fr 1fr}
    .q-cond{grid-template-columns:1fr 1fr}
    .settings-grid{grid-template-columns:1fr}
  }
  @media (max-width:560px){
    .est-form{grid-template-columns:1fr}
    .q-cli,.q-cond{grid-template-columns:1fr}
    /* No modal de custo, o campo vai pra linha inteira abaixo do rótulo */
    .cf-row{align-items:stretch}
    .cf-row label{flex:1 1 100%}
    .cf-row input,.cf-row select{flex:1 1 100%; width:100%; text-align:left}
    .cf-u{width:auto}
  }
</style>
</head>
  ...
  box.innerHTML=`
    ...
    <div class="q-box">
      ...
      <div class="est-form">
        ...
        <label>Quantidade<span class="est-qtd-wrap">
          <input id="estKg" type="text" inputmode="decimal" placeholder="Ex: 1000 g" class="est-qtd-input">
          <select id="estUnidade" class="est-unidade">
            <option value="kg" selected>kg</option>
            <option value="g">gramas</option>
          </select></span></label>
        <label>Preço do rolo (R$/kg)<input id="estPreco" type="text" inputmode="decimal" placeholder="Ex: 120,00"></label>
      </div>
      ...
      <p class="est-hint">Escolha a unidade ao lado da quantidade: <b>kg</b> (ex: 1 = 1 kg, 0,5 = meio quilo) ou <b>gramas</b> (ex: 300 = 300 g). Se a marca+tipo+cor já existir, a quantidade é somada ao saldo atual.</p>
    </div>`;
}
...
async function estoqueSalvar(){
  ...
  // le a quantidade e converte para kg (o servidor trabalha em kg)
  let qtdTexto=(g("estKg").value||"").trim().replace(",",".");
  let qtd=parseFloat(qtdTexto);
  if(isNaN(qtd)){ alert("Informe a quantidade."); return; }
  if(g("estUnidade").value==="g"){ qtd=qtd/1000; }   // gramas -> kg
  const body={marca:g("estMarca").value,tipo:g("estTipo").value,cor:g("estCor").value,
    cor_hex:g("estCorHex").value,kg:String(qtd),preco_kg:g("estPreco").value};
...
async function formCusto(){
  ...
  const blocoEstoque=temEstoque?`
      ...
      <label class="cf-desc"><input type="checkbox" id="cf_descontar" checked onchange="toggleDescontar()"> Descontar do estoque</label>
      <div class="cf-row" id="cf_precomanual_row" style="display:none">
...
function toggleDescontar(){
  // Quando o usuário DESMARCA "descontar do estoque", mostra o campo de custo
  // manual (porque o preço não virá mais do item do estoque). Ao marcar, esconde.
  const desc=document.getElementById("cf_descontar");
  const row=document.getElementById("cf_precomanual_row");
  if(!desc || !row) return;
  row.style.display = desc.checked ? "none" : "flex";
  custoPreview();
}
function custoPreview(){
  ...
  if(!out) return;
  // preço do kg: se vai descontar do estoque, usa o preço do item escolhido;
  // se NÃO vai descontar, usa o campo de custo manual.
  let pkg=0;
  const selEst=document.getElementById("cf_estoque");
  const desc=document.getElementById("cf_descontar");
  const vaiDescontar = desc ? desc.checked : false;
  if(selEst && selEst.value && vaiDescontar){
...
async function salvarCusto(){
  ...
  if(selEst){
    ...
    // só é obrigatório escolher o filamento se for descontar do estoque
    if(descontar && !selEst.value){ alert("Escolha o filamento do estoque para descontar (ou desmarque \"Descontar do estoque\")."); return; }
    const it=(window._custoEstoque||[]).find(x=>String(x.id)===selEst.value);
    if(descontar){
    ...
    // não descontar: usa o CUSTO MANUAL que o usuário preencheu
    const precoManual=g("cf_preco_kg");
    const it2=it||{preco_kg:0,tipo:""};
    const body={name,file:jobKey(st),material:it2.tipo,preco_kg:precoManual||it2.preco_kg,
...
async function testarFlashforge(){
  ...
  try{
    ...
    if(ad.ok){ closeAdd(); }  // a lista atualiza sozinha via WebSocket
    ...
    m.innerHTML=`
      ...
      <div class="wiz-body">
        <div class="auto-detect-box">
          ...
          <div class="wiz-hint" style="margin:0">
            <b>1.</b> Na tela da impressora: <b>Configurações → Rede → Modo LAN</b> → ative.<br>
            <b>2.</b> Anote o <b>código (Printer ID)</b> que aparece nessa tela de rede.<br>
            <b>3.</b> Anote o <b>número de série (SN)</b> — fica em Configurações → Informações, ou no adesivo atrás da impressora. Digite com o <b>SN</b> na frente.
          </div>
        </div>
        ...
        <label>Número de série <small style="color:var(--warn)">(obrigatório · com o SN na frente)</small></label>
...
if __name__ == "__main__":
    ...
    # Verifica a licença
    lic = refresh_license()
    # Se ativou pela Central (tem licenca_id salvo), considera ativado mesmo
    # que o núcleo offline não esteja presente.
    _cc = _central_config()
    if _cc.get("url") and _cc.get("licenca_id") and not lic.get("ok"):
        LICENSE_STATE["ok"] = True
        LICENSE_STATE["cliente"] = _cc.get("licenca_id")
        try:
            LICENSE_STATE["fingerprint"] = machine_fingerprint()
        except Exception:
            pass
        print(f"[licenca] ativa pela Central — {_cc.get('licenca_id')}")
        lic = LICENSE_STATE
    if lic.get("ok"):
