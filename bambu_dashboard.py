...
def _reiniciar_sistema():
    """Relança o sistema (novo processo) e encerra o atual. Usado após
    atualizar, para que a versão nova entre em uso sem o usuário precisar
    ...
    def _fazer():
        import time
        time.sleep(1.5)  # dá tempo da resposta HTTP chegar ao navegador
        try:
            exe = sys.executable
            script = str(Path(__file__).resolve())
            # remove flags de controle que não valem no relançamento
            args = [a for a in sys.argv[1:] if a != "--init-only"]
            flags = 0
            if platform.system() == "Windows":
                # DETACHED_PROCESS: o novo processo sobrevive ao encerramento deste
                flags = getattr(subprocess, "DETACHED_PROCESS", 0)
            subprocess.Popen([exe, script] + args, close_fds=True,
                             creationflags=flags)
        except Exception as exc:
            print("[update] falha ao relançar:", exc)
            return
        # encerra o processo atual para liberar a porta 8000
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .q-itens th:nth-child(4),.q-itens td:nth-child(4){width:118px}
  .q-tempo{display:flex; gap:.25rem; align-items:center}
  .q-tempo input{flex:1; min-width:0}
  .q-tempo .q-tempo-u{width:52px; flex:0 0 52px; padding:.42rem .3rem}
...
async function reiniciarEReconectar(){
  ...
  ov.style.cssText="position:fixed;inset:0;background:#0a0e16;z-index:99999;"+
    "display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1.2rem;color:#cdd6e4";
  ov.innerHTML=`<div class="rem-spin"></div>
    <div style="font-size:1.05rem">Aplicando a nova versão…</div>
    <div style="font-size:.85rem;color:#7a8699">O sistema volta em instantes, é só aguardar.</div>`;
  ...
  const tentar=async()=>{
    ...
    if(tentativas<40) setTimeout(tentar,1500);
    else{
      ov.innerHTML=`<div style="font-size:1rem">Quase lá…</div>
        <div style="font-size:.85rem;color:#7a8699">Se esta tela não sair sozinha,
        atualize a página (F5).</div>`;
    }
  };
  setTimeout(tentar, 4000);  // dá tempo do processo reiniciar
}
