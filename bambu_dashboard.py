...
def _reiniciar_sistema():
    ...
    8000 ser liberada antes de subir a nova instância. Assim a versão nova
    entra em uso sem o usuário precisar fechar e abrir manualmente, e sem o
    conflito de porta que fazia a tela de atualização ficar em loop."""
    import sys
    import subprocess
    import threading
    ...
    def _fazer():
        ...
        # 1) lança um relançador que ESPERA a porta liberar e só então sobe o app.
        #    Assim não há disputa: este processo morre e libera a porta primeiro.
        relauncher = (
            ...
            " r=False\n"
            " try:\n"
            "  s.bind(('127.0.0.1',8000)); r=True\n"
            " except OSError:\n"
            "  r=False\n"
            " finally:\n"
            "  s.close()\n"
            " return r\n"
            "for _ in range(60):\n"   # espera até ~30s a porta liberar
            " if livre(): break\n"
            " time.sleep(0.5)\n"
            f"subprocess.Popen([{exe!r},{script!r}]+{args!r})\n"
        )
        try:
            subprocess.Popen([exe, "-c", relauncher], close_fds=True,
                             creationflags=flags)
        except Exception as exc:
            print("[update] falha ao preparar o relançamento:", exc)
            return
        # 2) dá tempo da resposta HTTP chegar ao navegador e encerra este processo,
        #    liberando a porta 8000 para o relançador subir a versão nova.
        time.sleep(1.2)
...
def orc_calcular(orc, cfg=None):
    ...
    for it in orc.get("itens", []):
        ...
        # custo do filamento: o dono informa direto (por unidade)
        c_material = _f(it.get("valor_filamento"))
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .q-itens{min-width:820px}
  /* cada campo com largura mínima própria (o navegador respeita melhor assim) */
  .q-itens input[data-f=peso_g]{width:52px}
  .q-itens select[data-f=peso_unid]{width:46px; padding:.42rem .2rem}
  .q-itens input[data-f=tempo_min]{width:64px}
  .q-itens input[data-f=valor_filamento]{width:80px}
  .q-itens input[data-f=qtd]{width:50px}
  .q-itens input[data-f=valor_venda]{width:82px}
  .q-tempo{display:flex; gap:.25rem; align-items:center}
  .q-tempo .q-tempo-u{width:44px; flex:0 0 44px; padding:.42rem .2rem}
  .q-databox{padding:.9rem 1.1rem}
  .q-datalbl{display:flex; align-items:center; gap:.6rem; font-size:.9rem; color:var(--ink); font-weight:600}
  .q-datalbl input{background:#0a0e16; border:1px solid var(--hair); border-radius:8px;
    padding:.5rem .7rem; color:var(--ink); font-size:.9rem; font-family:inherit}
  ...
  .qt-margem{margin-top:.9rem; padding:.8rem .9rem; background:rgba(120,140,170,.08);
    border:1px dashed var(--hair); border-radius:10px}
  .qt-mrow{display:flex; justify-content:space-between; align-items:baseline; font-size:.85rem;
    color:var(--muted); margin-bottom:.3rem}
  .qt-mrow b{font-family:'JetBrains Mono',monospace; color:var(--ink)}
  .qt-lucro b{color:var(--done)}
  .qt-mhint{font-size:.72rem; color:var(--faint); margin-top:.4rem; line-height:1.4}
...
<div class="app">
  ...
  <div class="content">
    <header class="topbar">
      ...
      <div class="tb-brand">3DWORK — FARM DE IMPRESSORAS 3D</div>
...
const PAGES={dashboard:"Dashboard",printers:"Impressoras",projects:"Projetos",
  reports:"Relatórios",quotes:"Orçamentos",calc:"Calculadora",settings:"Configurações",mural:"Mural de atualizações"};
  ...
  box.innerHTML=`
    <div class="mural-wrap">
      <div class="mural-intro">
        ...
        <div>
          <div class="mural-intro-t">O que há de novo no 3DWORK</div>
...
function qEditor(){
  ...
  const itens=o.itens.map((it,i)=>`
    <tr>
      ...
      <td><input class="qi qnum" data-f="valor_filamento" data-i="${i}" type="number" min="0" step="0.01" value="${it.valor_filamento??""}" placeholder="0,00" oninput="qTouch()" title="Quanto custa o filamento desta peça"></td>
    ...
    <div class="q-box">
      ...
      <div class="q-tablewrap"><table class="q-table q-itens">
        <thead><tr>
          ...
          <th title="Quanto custa o filamento desta peça (por unidade)">Valor filamento (R$)</th>
          <th>Qtd</th>
...
async function aplicarUpdate(){
  if(!confirm("Atualizar o sistema para a versão "+((updateInfo&&updateInfo.nova)||"nova")+
    "?\n\nO sistema vai atualizar e reiniciar sozinho. Leva alguns segundos.")) return;
  ...
  try{
    ...
    if(d.ok){
      if(b) b.innerHTML=`<span>✅ Atualizado para a versão <b>${d.nova}</b>!
        Reiniciando o sistema…</span>`;
      // pede o reinício do sistema e reconecta o navegador sozinho
      reiniciarEReconectar();
    }else{
...
async function reiniciarEReconectar(){
